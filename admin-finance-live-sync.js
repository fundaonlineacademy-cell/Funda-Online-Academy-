...
async function sync(){if(busy||!dashboardVisible())return;busy=true;try{const c=client();if(!c)return;const [en,co,pay,cash]=await Promise.all([
 ...
 const bookedTuition=approved.reduce((a,x)=>a+n(x.amount),0);
