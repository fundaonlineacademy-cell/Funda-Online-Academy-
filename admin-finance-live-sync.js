...
function patchFinanceCopy(x){const root=document.querySelector('.finx');if(!root)return;const contracted=n(x.contracted_tuition??x.enrolment_snapshot_tuition??x.approved_tuition),outstanding=n(x.contracted_outstanding_tuition??x.outstanding_tuition),paid=n(x.contracted_verified_against_approved??x.verified_against_approved);const cards=[...root.querySelectorAll('.finCard')];if(cards[0]){const strong=cards[0].querySelector('strong'),span=cards[0].querySelector('span');if(strong)strong.textContent=money(contracted);if(span)span.textContent='Contracted approved tuition · learner-agreed fees'}if(cards[2]){const strong=cards[2].querySelector('strong'),span=cards[2].querySelector('span');if(strong)strong.textContent=money(outstanding);if(span)span.textContent='Outstanding approved receivables'}const rows=[...root.querySelectorAll('.finPanel:first-of-type .finRow')];if(rows[0]){rows[0].querySelector('span').textContent='Contracted approved tuition';rows[0].querySelector('b').textContent=money(contracted)}if(rows[1]){rows[1].querySelector('span').textContent='Verified against contracted tuition';rows[1].querySelector('b').textContent=money(paid)}if(rows[3]){rows[3].querySelector('span').textContent='Outstanding approved receivables';rows[3].querySelector('b').textContent=money(outstanding)}const infos=[...root.querySelectorAll('.finInfo')];const priceInfo=infos[infos.length-1];if(priceInfo)priceInfo.innerHTML=`<b>Price integrity:</b> contractual receivables remain ${money(contracted)} while today's catalogue values the same approved enrolments at ${money(x.approved_tuition)} (variance ${money(x.catalogue_variance)}). Existing learner balances are not rewritten by a later course-price change.`;const hero=root.querySelector('.finHero p');if(hero)hero.textContent='One governed financial source for learner contracts, verified payments, receivables, cashbook and executive reporting.'}function patchExecutive(x){
