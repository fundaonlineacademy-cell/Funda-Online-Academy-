...
function renderSupportHub(){
 ...
 if(al)al.innerHTML=notifications.length?notifications.map(x=>{
   ...
   return '<article class="announcementCard '+(x.pinned?'pinnedAnnouncement':'')+'"><div class="announcementTop"><div><b>'+esc(x.title)+'</b><span class="meta">'+esc(String(x.category||'programme').toUpperCase())+' · '+fmt(x.created_at)+'</span></div>'+(x.pinned?'<span class="badge warn">PINNED</span>':'')+'</div><div class="announcementFrom"><span>FROM</span><strong>'+esc(from)+'</strong><small>'+esc(name)+(title?' · '+esc(title):'')+'</small></div><p>'+esc(x.message)+'</p><div class="announcementScope">'+esc(x.audience==='all_funda'?'EVERYONE IN FUNDA':x.audience==='ambassadors'||x.audience==='all_ambassadors'?'ALL AMBASSADORS':x.audience==='individual_ambassador'?'DIRECT TO YOU':'AMBASSADOR NOTICE')+(x.priority&&x.priority!=='normal'?' · '+esc(String(x.priority).toUpperCase()):'')+'</div></article>'
