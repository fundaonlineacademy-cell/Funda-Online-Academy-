(()=>{
 ...
 function ensureMarketingSourceOption(){
  const select=document.getElementById('whereHeard');if(!select)return;
  const value='Influencer / Brand Ambassador';
  if([...select.options].some(option=>option.value===value||option.textContent.trim()===value))return;
  const option=document.createElement('option');option.value=value;option.textContent=value;
  const other=[...select.options].find(item=>item.textContent.trim()==='Other');
 ...
 function init(){addTerms();protectSubmit();ensureMarketingSourceOption();}
