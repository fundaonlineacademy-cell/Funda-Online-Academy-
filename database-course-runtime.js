...
const hasAssessment=(n,t)=>!!assessment(n,t);
...
async function load(force=false){
 ...
 data.loading=(async()=>{
  const mr=await db.from('course_modules').select('id,course_id,module_number,module_name,description,learning_outcomes').eq('course_id',id).order('module_number',{ascending:true});
  if(mr.error)throw mr.error;data.modules=mr.data||[];
  const mids=data.modules.map(x=>x.id);if(!mids.length){data.id=id;return}
  const progressIds=[...new Set([state.user?.id,state.studentId].filter(Boolean))];
  ...
  const aids=data.assessments.map(x=>x.id);
  if(aids.length){
    const rr=await db.from('assessment_attempts').select('assessment_id,passed,attempt_number,percentage').eq('student_id',state.user.id).in('assessment_id',aids);
    if(rr.error)throw rr.error;data.attempts=rr.data||[];
  }else data.attempts=[];
  data.id=id;
...
function sidebar(){
 ...
 host.innerHTML=data.modules.map(m=>{
   const n=Number(m.module_number),ls=lessons(n),open=unlocked(n),dc=doneCount(n),fp=passed(n,'formative'),sp=passed(n,'summative');
   const hasF=hasAssessment(n,'formative'),hasS=hasAssessment(n,'summative');
   let html=ls.map(l=>{
     const u=Number(l.lesson_number),active=n===state.module&&u===state.unit;
     return `<button class="unit ${active?'active':''} ${open?'':'locked'}" data-m="${n}" data-u="${u}" ${open?'':'disabled'}><span class="dot">${complete.has(String(l.id))?'✓':active?'▶':u}</span><span>${text(l.title)}</span></button>`
   }).join('');
   if(hasF){
     const fl=!open||dc<ls.length;
     html+=`<button class="unit ${fl?'locked':''}" data-m="${n}" data-u="9" data-type="formative" ${fl?'disabled':''}><span class="dot">${fp?'✓':'9'}</span><span>Formative Assessment</span></button>`;
   }
   if(hasS){
     const sl=!open||dc<ls.length||(hasF&&!fp);
     const number=hasF?10:9;
     html+=`<button class="unit ${sl?'locked':''}" data-m="${n}" data-u="${number}" data-type="summative" ${sl?'disabled':''}><span class="dot">${sp?'✓':number}</span><span>Summative Assessment</span></button>`;
   }
   return `<div class="module"><h3>Module ${n} · ${text(m.module_name)}</h3><div style="font-size:10px;color:#64748b;margin-bottom:8px">${dc}/${ls.length} lessons complete</div>${html}</div>`;
 }).join('');
 host.querySelectorAll('.unit:not(.locked)').forEach(b=>b.onclick=()=>{
   state.module=Number(b.dataset.m);state.unit=Number(b.dataset.u);
   const t=b.dataset.type;
   if(t){window.location.assign(`module-assessment.html?course=${encodeURIComponent(state.course.id)}&module=${state.module}&type=${t}`)}
   else{render();if(innerWidth<761)$('modulePanel')?.classList.remove('open')}
 })
}
function lessonView(){
 ...
 const nk=`funda-note-${state.course.id}-${l.id}`;$('lessonNotes').value=localStorage.getItem(nk)||'';
 $('saveNotes').onclick=()=>{localStorage.setItem(nk,$('lessonNotes').value);show('Lesson notes saved on this device.',true)};
 $('prevLesson').disabled=state.module===1&&Number(l.lesson_number)===1;
 $('prevLesson').onclick=()=>{if(Number(l.lesson_number)>1)state.unit=Number(l.lesson_number)-1;else if(state.module>1){state.module--;state.unit=lessons(state.module).length||1}render();scrollTo(0,0)};
 $('completeLesson').onclick=async()=>{
   const b=$('completeLesson');b.disabled=true;b.textContent='Saving…';
   try{
     if(!complete){
       const now=new Date().toISOString();
       const r=await db.from('lesson_progress').upsert({student_id:state.user.id,lesson_id:l.id,completed:true,completed_at:now,updated_at:now},{onConflict:'student_id,lesson_id'});
       if(r.error)throw r.error;await load(true)
     }
     if(Number(l.lesson_number)<ls.length)state.unit=Number(l.lesson_number)+1;
     else{
       const nextType=hasAssessment(state.module,'formative')?'formative':hasAssessment(state.module,'summative')?'summative':null;
       if(nextType){window.location.assign(`module-assessment.html?course=${encodeURIComponent(state.course.id)}&module=${state.module}&type=${nextType}`);return}
       state.unit=Number(l.lesson_number);
     }
     render();scrollTo(0,0)
   }catch(e){console.error(e);show('Lesson completion could not be saved.');b.disabled=false;b.textContent='Complete Lesson & Continue →'}
 }
}
async function draw(){
 try{
  await load();
  if(!data.modules.length){$('course-content').innerHTML='<div class="message">No modules are configured for this course.</div>';return}
  if(!unlocked(state.module)){state.module=1;state.unit=1}
  $('sidebarCourse').textContent=state.course.title||'Selected Course';
  const p=progress();$('progressText').textContent=`${p}%`;$('progressFill').style.width=`${p}%`;
  sidebar();lessonView()
 }catch(e){console.error(e);$('course-content').innerHTML=`<div class="message">Unable to load course content: ${text(e.message||'Please try again.')}</div>`}
}
...
function bootDatabaseCourse(){
 bootTries++;
 if(state.course&&state.user){render();return}
 if(bootTries<50)setTimeout(bootDatabaseCourse,200);
}
