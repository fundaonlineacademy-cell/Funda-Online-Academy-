...
function targetsPanel(){
  ...
  return `<div class="acGrid">
    ...
    <div class="acPanel"><h3>Annual Planning Summary</h3><div class="acMeta">Annual revenue target: <b>${money(target)}</b></div><div class="acMeta">Monthly baseline: <b>${money(target/12)}</b></div><div class="acMeta">Budgeted revenue across 12 months: <b>${money(totals.revenue)}</b></div><div class="acMeta">Budgeted expenses: <b>${money(expenseBudget)}</b></div><div class="acMeta">Planned surplus: <b>${money(plannedSurplus)}</b></div><div class="acMeta">FY ${year}/${String(year+1).slice(-2)} runs 1 Oct ${year} - 30 Sep ${year+1}.</div><div class="acBar"><button class="acBtn alt" id="distributeAnnualTarget">Distribute Annual Target Across 12 Months</button><button class="acBtn" id="openManagementReports">Open Management Reports</button></div><div class="acMeta">Distribution changes only monthly revenue targets. It does not overwrite expense, Ambassador, staff or surplus budgets. Management Reports compare these approved plans with recognised actuals.</div></div>
  </div>
...
function wire(){
  ...
  if(tab==='targets'){
    ...
    $('openManagementReports')?.addEventListener('click',()=>{managementReportMode='financial_year';managementReportFyYear=firstFyStart();managementReportData=null;render('reports')});
