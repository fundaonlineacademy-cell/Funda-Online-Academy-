...
function payoutRows(){
 if(!payouts.length)return '<tr><td colspan="6" class="afmEmpty">No Ambassador payouts recorded yet.</td></tr>';
...
function block(){
 const t=totals();
 const opts=apps.filter(x=>x.status==='approved').map(x=>'<option value="'+x.id+'">'+esc(x.full_name)+'</option>').join('');
 return '<section id="ambFinanceAdmin" class="afm"><div class="afmHero"><div class="afmKicker">AMBASSADOR FINANCE CONTROL</div><h2>Ambassador Earnings, Banking & Payouts</h2><p>Commission becomes visible to an Ambassador only after Finance verifies the payment and Administration approves the student enrolment. The system blocks an invalid commission from being approved or paid.</p></div><div class="afmStats"><div class="afmStat"><b>'+money(t.approved)+'</b><small>CONFIRMED EARNINGS</small></div><div class="afmStat"><b>'+t.review+'</b><small>RECORDS REQUIRING REVIEW</small></div><div class="afmStat"><b>'+money(t.paid)+'</b><small>PAYOUTS PAID</small></div><div class="afmStat"><b>'+t.banks+'</b><small>BANK ACCOUNTS TO VERIFY</small></div></div><div class="afmPanel"><h3>Banking Verification</h3><p class="afmPanelLead">Confirm that the account holder and bank details are correct before marking the account verified.</p><div class="afmWrap"><table class="afmTable"><thead><tr><th>Ambassador</th><th>Bank</th><th>Account Holder</th><th>Account</th><th>Branch</th><th>Status</th><th>Action</th></tr></thead><tbody>'+bankRows()+'</tbody></table></div></div><div class="afmPanel"><h3>Earnings Verification</h3><div class="afmWarn"><b>Controlled confirmation:</b> A blocked commission cannot be changed to approved or paid. Correct the linked payment or student enrolment first; the ledger then updates automatically.</div><div class="afmWrap"><table class="afmTable"><thead><tr><th>Ambassador</th><th>Type</th><th>Qualifying Revenue</th><th>Amount</th><th>Month</th><th>Status</th><th>Verification</th><th>Audit Note</th><th>Action</th></tr></thead><tbody>'+earningRows()+'</tbody></table></div></div><div class="afmPanel"><h3>Payout Queue & History</h3><p class="afmPanelLead">Record a payout only against confirmed earnings and verified banking details.</p><div class="afmForm" style="margin:10px 0 14px"><select id="afmPayApp"><option value="">Choose Ambassador</option>'+opts+'</select><input id="afmPayAmount" type="number" min="0.01" step="0.01" placeholder="Amount"><input id="afmPayRef" placeholder="Payment reference"><input id="afmPayDate" type="date"><select id="afmPayStatus"><option>scheduled</option><option>processing</option><option>paid</option></select><button class="afmBtn gold" id="afmAddPay">Record Payout</button></div><div class="afmWrap"><table class="afmTable"><thead><tr><th>Ambassador</th><th>Amount</th><th>Date</th><th>Reference</th><th>Status</th><th>Action</th></tr></thead><tbody>'+payoutRows()+'</tbody></table></div></div></section>';
}
...
function wire(){
 ...
 root.onclick=async e=>{
  ...
  if(b.dataset.paySave){
   const sel=root.querySelector('[data-pay-select="'+b.dataset.paySave+'"]'),patch={status:sel.value};
   if(sel.value==='paid')patch.payment_date=new Date().toISOString().slice(0,10);
   const q=await db.from('ambassador_payouts').update(patch).eq('id',b.dataset.paySave);
   if(q.error)return alert(q.error.message);return refresh();
  }
  if(b.id==='afmAddPay'){
   ...
   if(!aid||!amt)return alert('Ambassador and payout amount are required.');
...
async function marketingSummary(){
 ...
 s.innerHTML='<h3>Ambassador Programme Financial Snapshot</h3><div class="afmStats"><div class="afmStat"><b>'+money(t.approved)+'</b><small>CONFIRMED EARNINGS</small></div><div class="afmStat"><b>'+t.review+'</b><small>RECORDS REQUIRING REVIEW</small></div><div class="afmStat"><b>'+money(t.paid)+'</b><small>PAYOUTS PAID</small></div><div class="afmStat"><b>'+t.banks+'</b><small>BANK ACCOUNTS TO VERIFY</small></div></div><p class="afmMeta">Full banking verification, controlled earning confirmation and payout processing are available under Finance & Accounting.</p>';
 host.appendChild(s);
}
