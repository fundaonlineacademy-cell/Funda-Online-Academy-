...
function decorate(code){if(!code)return;document.querySelectorAll('a[href]').forEach(a=>{try{let u=new URL(a.getAttribute('href'),location.href);if(u.origin!==location.origin)return;if(!/(courses-public|course-view|create-account|auth|login|onboarding)\.html$/i.test(u.pathname))return;if(!u.searchParams.get('ref'))u.searchParams.set('ref',code);a.href=u.pathname+u.search+u.hash}catch(e){}})}
