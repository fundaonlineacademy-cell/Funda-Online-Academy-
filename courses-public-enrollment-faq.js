...
function install(){
  ...
  if(existing){
    const details=existing.closest('details');
    const answer=details?.querySelector('p');
    if(answer)answer.textContent='You can enroll at any time. Funda Online Academy accepts registrations every day. Once your registration and payment have been verified and approved, your course access is opened and you can begin learning.';
    return;
  }
  ...
  item.innerHTML='<summary class="cursor-pointer flex justify-between gap-3 font-semibold text-sm text-[#071D49]">When can I enroll and start my course?<span class="faq-arrow text-[#C99A2E]">＋</span></summary><p class="mt-3 text-sm text-slate-600 leading-6">You can enroll at any time. Funda Online Academy accepts registrations every day. Once your registration and payment have been verified and approved, your course access is opened and you can begin learning.</p>';
