...
function leaderboardModal(){
  let rows=performance.map((x,i)=>`<tr><td><b>#${x.performance_position}</b>${i===0&&performance.length>1?' · Leader':''}</td><td><b>${esc(x.full_name)}</b><div class="ampMeta">${esc(x.current_rank)} · ${esc(label(x.account_status))}</div></td><td>${x.valid_referrals_month}<div class="ampMeta">${x.valid_referrals_lifetime} lifetime · ${x.disqualified_referrals} disqualified</div></td><td>${x.approved_students_month}</td><td>${money(x.verified_revenue_month)}<div class="ampMeta">${money(x.lifetime_verified_revenue)} lifetime</div></td><td>${money(x.confirmed_commission_month)}<div class="ampMeta">${money(x.monthly_performance_payment)} performance payment</div></td><td>${money(x.paid_out)}</td><td><button class="ampBtn alt" data-referrals="${x.application_id}">View Referrals</button></td></tr>`).join('');
  modal('Ambassador Performance Leaderboard',month+' · Ranking: verified qualifying revenue → approved referred Students → valid referrals.',`<div class="ampTableWrap"><table class="ampTable"><thead><tr><th>Position</th><th>Ambassador</th><th>Referrals</th><th>Approved Students</th><th>Verified Revenue</th><th>Confirmed Commission</th><th>Paid Out</th><th>Evidence</th></tr></thead><tbody>${rows||'<tr><td colspan="8">No active Ambassador performance records.</td></tr>'}</tbody></table></div><div class="ampSignal">The leaderboard is operational performance only. It does not automatically approve earnings, payouts or prizes.</div>`);
