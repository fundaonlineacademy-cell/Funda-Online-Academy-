...
function add(){
 ...
 if(!/business health overview/i.test(title))return;
 const card=document.createElement("section");card.id="fundaAdminHouseRules";card.style.cssText="background:#fff;border:1px solid #dbe4ee;border-radius:16px;padding:16px;margin:22px 0 4px;box-shadow:0 6px 20px rgba(7,27,49,.06)";
 card.innerHTML='<div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-size:9px;font-weight:900;letter-spacing:.13em;color:#a27c24">ADMIN GOVERNANCE</div><h3 style="margin:4px 0;color:#071b31">Admin & Staff House Rules</h3><p style="margin:0;color:#66758a;font-size:11px;line-height:1.5">Operational rules for protecting students, records, finances and the Academy.</p></div><button id="openAdminRules" style="border:0;border-radius:10px;background:#071b31;color:#fff;padding:10px 14px;font-size:10px;font-weight:800;cursor:pointer">View Rules</button></div>';
...
function watch(){
 ...
 if(!view)return setTimeout(watch,300);
 new MutationObserver(()=>setTimeout(add,80)).observe(view,{childList:true,subtree:false});
 setTimeout(add,80);
}
