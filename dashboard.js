...
async function loadAvailableCourses(studentId) {
  ...
  try {
    ...
    const {data:existing,error}=await db.from("enrollments").select("course_id,enrollment_status")
...
async function enrolStudent(studentId,courseId,button) {
  ...
  try {
    const {data:existing,error:checkError}=await db.from("enrollments").select("id,enrollment_status")
