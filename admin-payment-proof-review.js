...
`;
  ...
  function init(){
    ...
    document.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('aprModal'))closeDecision();});window.addEventListener('focus',()=>{if(financeActive()&&!$('aprModal'))load(false,true);});document.addEventListener('funda:admin-manual-refresh',()=>{if(!document.hidden&&financeActive()&&!$('aprModal'))load(true,true);});document.addEventListener('funda:admin-live-change',event=>{if(['payments','enrollments'].includes(String(event.detail?.table||''))&&financeActive()&&!$('aprModal'))load(true,true);});setTimeout(mount,1200);
  }
